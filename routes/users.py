...
def location(id):     
    ...
    try: 
        ...
        return resp.error(400, 'MISSED_FIELD', 'Missed required field: {}'.format(e.args[0]))
        
    if not (0.0 <= lat <= 90.0): 
        return resp.error(400, 'BAD_COORDINATES', 'Bad latitude value: {}'.format(lat))

    if not (0.0 <= lng <= 180.0):
        return resp.error(400, 'BAD_COORDINATES', 'Bad longitude value: {}'.format(lng))
